...
pub type ClientMessage {
  Authenticate(id_token: String, session_token: String)
...
pub fn encode_server_message(msg: ServerMessage) -> String {
  let json_obj = case msg {
    ...
    Authenticated(player_id, display_name, email, session_token, ftue_completed) ->
      json.object([
        ...
        #("ftue_completed", json.bool(ftue_completed)),
      ])
    ...
    FtueCompletedAck ->
      json.object([
        #("type", json.string("ftue_completed_ack")),
      ])
  }
...
fn encode_player_stats(s: db.PlayerStats) -> json.Json {
  json.object([
    ...
    #("ftue_completed", json.bool(s.ftue_completed)),
...
fn encode_team_state(s: MatchTeamState) -> json.Json {
  let targets_entries =
    ...
    |> dict.to_list()
    |> list.map(fn(pair) {
      let #(caster_slot, enemy_slot) = pair
      #(int.to_string(caster_slot), json.int(enemy_slot))
    })
  json.object([
    ...
    #("hero_targets", json.object(targets_entries)),
  ])
}
...
fn encode_hand_slot(s: MatchHandSlot) -> json.Json {
  case actions.get_action_def(s.action_slug) {
    Ok(action) ->
      json.object([
        #("match_id", json.string(s.match_id)),
        #("team", json.int(s.team)),
