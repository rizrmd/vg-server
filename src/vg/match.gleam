...
pub type MatchActorState {
  MatchActorState(
    ...
    dot_tick_at: Int,
    // Timestamp of the last DoT/Hot tick application
  )
}
...
fn handle_message(
  ...
  case message {
                ...
                Ok(hand_slot) -> {
                  ...
                  case get_hero_by_slot(state.heroes, team, caster_slot) {
                    Ok(caster) -> {
                      ...
                      case actions.get_action_def(hand_slot.action_slug) {
                        Ok(action) -> {
                          let target_result =
                            resolve_target(
                              state.heroes,
    ...
    SetHeroTarget(player_id, caster_slot, target_slot, reply_to) -> {
      case get_player_team(state, player_id) {
        Ok(team) -> {
          case dict.get(state.team_states, team) {
            Ok(team_state) -> {
              let new_targets =
                dict.insert(team_state.hero_targets, caster_slot, target_slot)
              let new_team_state =
                MatchTeamState(..team_state, hero_targets: new_targets)
              let new_team_states =
                dict.insert(state.team_states, team, new_team_state)
              process.send(reply_to, Ok(Nil))
              actor.continue(
                MatchActorState(..state, team_states: new_team_states),
              )
            }
            Error(_) -> {
              process.send(reply_to, Error("Team state not found"))
    ...
    Tick(now, reply_to) -> {
      case state.match.phase {
        Active -> {
          ...
          let #(heroes_after_ticks, new_dot_tick_at) =
            case now - state.dot_tick_at >= match_logic.dot_tick_interval_ms {
              True -> {
                let ticked =
                  match_logic.apply_dot_hot_ticks(new_heroes, new_statuses, now)
                #(ticked, state.dot_tick_at + match_logic.dot_tick_interval_ms)
              }
              False -> #(new_heroes, state.dot_tick_at)
            }

          // Check win condition after ticks
          let winner = match_logic.check_win_condition(heroes_after_ticks)
    ...
    ReplaceEnemies(enemy_slugs, reply_to) -> {
      let now = get_timestamp()
      // Remove team 2 heroes
      let team1_heroes =
        dict.filter(state.heroes, fn(_id, hero) { hero.team == 1 })
      // Spawn new team 2 heroes
      let new_team2 =
        spawn_heroes_for_team(state.match.match_id, 2, enemy_slugs, now)
      let all_heroes =
        dict.merge(team1_heroes, dict.from_list(new_team2))
      // Reset team 2 state
      let team2_state =
        match_logic.init_team_state(state.match.match_id, 2, now)
      let new_team_states =
        dict.insert(state.team_states, 2, team2_state)
      // Reroll hands for both teams
...
fn create_cast(
  ...
  let casting_time = case actions.get_action_def(action_slug) {
      ...
      dict.get(current_heroes, cast.target_hero_instance_id)
    {
      Ok(caster), Ok(target) -> {
        ...
        case
          ...
          actions.get_action_def(cast.action_slug)
        {
          Ok(caster_def), Ok(target_def), Ok(action) -> {
            ...
            let new_casts =
              dict.insert(current_casts, result.cast.cast_id, result.cast)
            let new_heroes =
              current_heroes
              |> dict.insert(result.caster.hero_instance_id, result.caster)
              |> dict.insert(result.target.hero_instance_id, result.target)

            // Remove expired/consumed statuses, then insert kept ones
            let statuses_pruned =
              list.fold(result.removed_ids, current_statuses, fn(acc, id) {
                dict.delete(acc, id)
              })
            let new_statuses_dict =
              list.fold(result.kept_statuses, statuses_pruned, fn(acc, s) {
...
pub fn replace_enemies(
  match_actor: Subject(Message),
  enemy_slugs: List(String),
) -> Result(Nil, String) {
  process.call(match_actor, waiting: 5000, sending: fn(subject) {
    ReplaceEnemies(enemy_slugs, subject)
  })
}
...
pub fn set_hero_target(
  ...
  process.call(match_actor, waiting: 5000, sending: fn(subject) {
    SetHeroTarget(player_id, caster_slot, target_slot, subject)
  })
}
