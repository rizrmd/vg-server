...
import vg/game_json.{
  ...
  ProfileUpdated, QueueMatchmaking, RerollHand, SetHeroTarget, StartTraining,
  StateUpdate, UpsertProfile,
}
...
fn authenticate_with_google(
  ...
  case id_token {
    "" -> {
      // Anonymous login with auto-generated username
      let now = get_timestamp()
      case db.create_anonymous_player(db_conn, now) {
        Ok(player) -> {
        ...
        Error(_) -> {
          send_server_message(
            conn,
            AuthError("DB_ERROR", "Failed to create anonymous player"),
          )
          mist.continue(state)
        }
      }
...
fn handle_authenticated_message(
  ...
  case msg {
    ...
    SetHeroTarget(match_id, caster_slot, target_slot) -> {
      case state.current_match_id {
        Some(current_id) if current_id == match_id -> {
          case match_registry.get_match(state.match_registry, match_id) {
            Ok(match_actor) -> {
              case
                match.set_hero_target(
                  match_actor,
                  state.player_id,
                  caster_slot,
                  target_slot,
                )
              {
                ...
                Error(err) -> {
                  send_server_message(conn, ServerError("TARGET_ERROR", err))
                  mist.continue(state)
                }
              }
            }
    ...
    ChangeTrainingEnemies(e1, e2, e3) -> {
      case state.current_match_id {
        Some(match_id) -> {
          case match_registry.get_match(state.match_registry, match_id) {
            Ok(match_actor) -> {
              let _ =
                match.replace_enemies(match_actor, [e1, e2, e3])
              send_match_state(conn, match_actor, state)
              mist.continue(state)
            }
            Error(_) -> {
        ...
        None -> {
          send_server_message(
            conn,
            ServerError("NOT_IN_MATCH", "Not in a training match"),
          )
          mist.continue(state)
        }
      }
    ...
    CompleteFtue -> {
      case state.db_conn {
        Some(db) -> {
          let now = get_timestamp()
          case db.mark_ftue_completed(db, state.player_id, now) {
            Ok(Nil) -> {
              send_server_message(conn, FtueCompletedAck)
              mist.continue(state)
            }
            Error(_) -> {
              send_server_message(
                conn,
                ServerError("DB_ERROR", "Failed to mark FTUE completed"),
              )
              mist.continue(state)
            }
          }
