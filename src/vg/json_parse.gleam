...
pub fn parse_client_message(json_str: String) -> Result(ClientMessage, Nil) {
  ...
  case msg_type {
    ...
    "set_hero_target" -> {
      use match_id <- result.try(get_json_string_field(json_str, "match_id"))
      use caster_slot <- result.try(get_json_int_field(json_str, "caster_slot"))
      use target_slot <- result.try(get_json_int_field(json_str, "target_slot"))
      Ok(SetHeroTarget(
        match_id: match_id,
        caster_slot: caster_slot,
        target_slot: target_slot,
    ...
    "get_match_state" -> {
      use match_id <- result.try(get_json_string_field(json_str, "match_id"))
      Ok(GetMatchState(match_id: match_id))
    }
    "change_training_enemies" -> {
      use e1 <- result.try(get_json_string_field(json_str, "enemy_hero_1"))
      use e2 <- result.try(get_json_string_field(json_str, "enemy_hero_2"))
      use e3 <- result.try(get_json_string_field(json_str, "enemy_hero_3"))
      Ok(ChangeTrainingEnemies(
        enemy_hero_1: e1,
        enemy_hero_2: e2,
        enemy_hero_3: e3,
      ))
    }
    "complete_ftue" -> Ok(CompleteFtue)
    _ -> Error(Nil)
  }
}
