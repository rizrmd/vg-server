...
All communication is via **WebSocket JSON messages**. Authentication via Google Sign-In or anonymous login is required before any gameplay messages.
...
**Option A: Anonymous login** (no account required)

Send an empty `id_token` to generate a guest account:

```json
{"type": "authenticate", "id_token": ""}
```

The server generates a random display name (e.g. `SwiftFox42`) and a persistent `player_id` (e.g. `anon_123456`). You can reconnect later using the returned `session_token`.
...
- Google accounts: `player_id` format is `g_<google_sub>`. Same account always gets the same ID.
- Anonymous accounts: `player_id` format is `anon_<6digits>`. Display name is auto-generated (e.g. `SwiftFox42`).
- `session_token` should be cached locally for reconnection (works for both Google and anonymous).
...
Send the previously returned `session_token`:

```json
{"type": "authenticate", "session_token": "<cached_session_token>"}
```

**Using Google ID token:**

Send the Google ID token again. If expired (~1 hour), refresh via Google Sign-In first.
...
For anonymous login, `email` is empty and `player_id` starts with `anon_`.
