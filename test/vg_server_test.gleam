...
pub fn action_definitions_exist_test() {
  let actions = actions.action_definitions()
...
pub fn fireball_exists_test() {
  case actions.get_action_def("fireball") {
    Ok(action) -> {
...
pub fn parse_cast_action_test() {
  let json =
    "{\"type\":\"cast_action\",\"match_id\":\"match123\",\"caster_slot\":1,\"hand_slot_index\":2,\"target_slot\":3}"
  case jp.parse_client_message(json) {
    Ok(msg) -> {
      case msg {
        gj.CastAction(match_id, caster_slot, hand_slot, target_slot) -> {
          should.equal(match_id, "match123")
          should.equal(caster_slot, 1)
          should.equal(hand_slot, 2)
          should.equal(target_slot, 3)
        }
        _ -> should.fail()
      }
    }
