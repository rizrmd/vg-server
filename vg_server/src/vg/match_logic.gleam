...
import vg/content
import vg/types.{
  type ActionDef, type HeroDef, type MatchCast, type MatchHero, type MatchStatus,
  type MatchTeamState, type Element, type TargetRule,
  Damage, DamageAndStatus, EnemySingle, Fire, Heal, Ice,
  Light, MatchCast, MatchHero, MatchStatus, MatchTeamState, Self, Shadow, Shield,
  Status, Wind, Active, Waiting, Finished, NoWinner, Team1, Team2,
  ShieldBuff, AttackBuff, Dot, Hot, Cleanse, Earth, Stun, AllySingle,
}
...
pub fn calculate_damage(
  ...
  // Get affinity values
  let caster_affinity = get_affinity_for_element(caster_hero_def, action.element)
  let target_affinity = get_affinity_for_element(target_hero_def, action.element)
  ...
  let defense_mitigation = 100.0 /. { 100.0 +. int.to_float(target_hero_def.defense) }
...
pub fn calculate_heal(
  ...
  let affinity_bonus = 1.0 +. { int.to_float(caster_hero_def.light_affinity) /. 100.0 }
...
pub fn regen_energy(team_state: MatchTeamState, now: Int) -> MatchTeamState {
  ...
  let new_energy = int.min(team_state.energy_max, team_state.energy + regen_amount)
...
pub fn resolve_cast(
  cast: MatchCast,  action: ActionDef,
  caster: MatchHero,
  target: MatchHero,
  caster_def: HeroDef,
  target_def: HeroDef,
  ...
  case action.effect_kind {
    Damage -> {
      let damage = calculate_damage(action, caster, target, caster_def, target_def)
    ...
    Shield -> {
      ...
      let status = create_shield_status(cast.match_id, target.hero_instance_id, shield, now + 5000, now)
      #(resolved_cast, caster, target, [status], shield)
    }
    Status -> {
      let status = create_status_from_action(action, cast.match_id, target.hero_instance_id, now)
...
fn generate_id(prefix: String, timestamp: Int) -> String {
  prefix <> "_" <> int.to_string(timestamp) <> "_" <> int.to_string(int.random(10000))
}
